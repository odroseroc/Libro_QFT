...
La renormalización de las teorías de campos resulta siempre necesaria por una simple razón. Cuando se escribe un Lagrangiano, los parámetros que se tienen no necesariamente se corresponden con las cantidades física. Es necesario aplicar todo el formalismo que hemos desarrollado para calcular cantidades que sean medibles y observables en el laboratorio.

En este proceso de pasar de algo abstracto -- como lo es un Lagrangiano -- a calcular observables es necesario tener en cuenta que existen cantidades física y otras cantidades puramente teóricas. Un ejemplo de ello es lo que llamaremos más adelante la \emph{masa desnuda} y su contraparte, la masa física. Es necesario conectar los dos conceptos porque, en general, no siempre coinciden. Ya hemos visto que en el Lagrangiano tenemos un término de masa que es proporcional al campo al cuadrado. Este término aparece aún para los quarks. Sin embargo, hay un inconveniente aquí: los quarks no tienen masa física, aún cuando tienen un término en el Lagrangiano que denominamos masa.

El proceso de conectar los parámetros en la teoría, que llamaremos \emph{parámetros desnudos} (bare parameters) es lo que llamaremos renormalización. En general, toda teoría, bien sea clásica o cuántica, requiere una renormalización. Sin embargo, el proceso nunca fue tan evidente como cuando, a comienzos de los años 1930 se empiezan a calcular cantidades a un loop y aparecieron infinitos ineludibles. Fue entonces que el proceso de renormalización se volvió crucial para que los cálculos tuvieran sentido físico, pues muchas veces los parámetros desnudos no estaban unívocamente definidos.

\section{Sistemática de la renormalización}

Para motivar el tema, iniciaremos con un cálculo proveniente de la QED~\footnote{QED: Quantum ElectroDynamics, electrodinámica cuántica} y un concepto que desarrolló Weinberg denominado \emph{grado superficial de divergencia} (superficial degree of divergence).

\subsection{Grado superficial de divergencia}

Para QED emplearemos las siguientes definiciones en un diagrama
\clearpage
...
La idea general de los diagramas irreducibles es que a partir de ellos se pueden construir diagramas más complejos. Todo diagrama se puede construir a partir de diagramas irreducibles. Son una especie de equivalente de los números primos para el conjunto de los números naturales. Recordemos, 

\begin{cabox}
	el concepto de grado superficial de divergencia será aplicado únicamente a diagramas irreducibles.
\end{cabox}

Veamos entonces de qué se trata este concepto.

Supongamos que se tiene una amplitud que involucra ciertas integrales de la forma
%
...
El diagrama que consideramos tendrá tantas integrales en $k$ como loops hayan y definiremos el grado superficial de divergencia como
%
\begin{equation}
\boxed{
	D = 4L - P_e - 2P_\gamma \,.
...
La razón de esta definición es la siguiente: sabemos que por cada loop que hay en el diagrama habrá una integral $\dd[4]k_i$. Estamos interesados en la dimensión, en unidades de momento, del integrando. En unidades de momento, cada diferencial tiene dimensión $4$, i.e., cada loop aportará un $4$ a la dimensión del integrando. Ahora, los propagadores fermiónicos son de la forma
%
\begin{equation*}
	\frac{\ii}{k - m} \,,
\end{equation*}

\noindent por lo que por cada propagador fermiónico, la dimensión en unidades de momento, disminuye en una unidad. Por su parte, los propagadores de fotones son de la forma
%
\begin{equation*}
	\frac{\ii}{k^2} \,,
\end{equation*}

\noindent haciendo que por cada propagador de fotones la dimensión en unidades de momento disminuya en dos unidades.

Considerando el grado de divergencia, usualmente lo que ocurrirá es lo siguiente:
%
\begin{profor}
    \begin{equation*}
	\begin{IEEEeqnarraybox}[][c]{ll}
		\text{Si } D > 0 \qc & \text{tendremos cantidades que divergen de la forma } \Lambda^D\,; \\
		%%%%
		\text{si } D = 0 \qc & \text{tendremos cantidades que divergen como} \log \Lambda \,; \\
		%%%%
...
\noindent donde $\Lambda$ es el límite superior de los momentos, y $\Lambda \rightarrow \infty$. Esto es lo que se denomina un corte (cutoff) de los momentos. Cabe anotar que esto no es una regla general, existen excepciones a lo que se ha dicho, no obstante, estas condiciones se cumplen en muchos casos para los diagramas irreducibles.
