...
El propósito de toda teoría física es calcular cantidades medibles **** redundante ***. En el formalismo de las teorías de campos aparecen elementos matemáticos que representan cantidades físicas, pero también otros que no son medibles, sino que son cantidades puramente abstractas que sólo existen como parte del aparato matemático. \textcolor{red}{revisar redacción}

Ejemplo de ello es la denominada \emph{masa desnuda}, una cantidad teórica asociada al parámetro de masa en el Lagrangiano. Dicho término siempre es proporcional al campo al cuadrado y aparece para todas las partículas masivas de la teoría. En el caso del modelo estándar, dicho parámetro aparece incluso en el caso de los quarks, aún cuando éstos no tienen masa física.

La esencia de la \textbf{renormalización} en teoría cuántica de campos consiste en conectar estas cantidades abstractas -- los prámetros desnudos -- que aparecen en el Lagrangiano con cantidades medibles. 
...
La idea general de los diagramas irreducibles es que a partir de ellos se pueden construir diagramas más complejos. Todo diagrama se puede construir a partir de diagramas irreducibles. Hay una similitud entre lo que son los diagramas irreducibles y los números primos para el conjunto de los naturales. Entonces, tengamos en cuenta lo siguiente: 

\begin{cabox}
	El concepto de grado superficial de divergencia será aplicado únicamente a diagramas irreducibles.
\end{cabox}

Con estos conceptos claros, procedamos ahora a explorar la idea de grado superficial de divergencia.

Típicamente, la integral para la amplitud de un diagrama en QED tiene la forma
%
...
El grado superficial de divergencia, como podemos ver, es la dimensión del integrando en unidades de momento. Hablando de manera muy general, un diagrama diverge en el ultravioleta si hay más potencias de momento en el numerador que en el denominador.
...
\begin{profor}
	...
	\begin{IEEEeqnarraybox}[][c]{ll}
		...
		%%%%
		\text{si } D < 0 \qc & \text{las cantidades irań como } \Lambda^{-\abs{D}} \text{ y son típicamente convergentes.}
	\end{IEEEeqnarraybox}
\end{equation*}
\end{profor}
...
Ahora, en QED el número de loops de un diagrama de Feynman es dado por
%
\begin{equation}
	L = P_e + P_\gamma - \qty(V - 1) \,.
\end{equation}

Esto proviene de las reglas de Feynman, que exigen que por cada propagador hay una integral en el momento correspondiente~\footnote{cf. David Griffiths. \emph{Introduction to Elementary Particles}. Second Edition. Wiley-VCH, 2008. Capítulos 6 y 7.}. No obstante, por cada vértice existe una delta de Dirac que elimina una de las integrales. Cuando se calcula la amplitud de un diagrama, también aparece una delta de Dirac que expresa la conservación global del momento lineal, la cual no se puede integrar como las demás. A esta delta, que no elimina una integral, se debe el $-1$ en el paréntesis. Finalmente, el número de loops es el número de integrales de cuatro dimensiones presentes en la expresión.

El número de vértices en un diagrama de QED es dado por
%
\begin{equation}
	V = 2P_\gamma + N_\gamma \,,
\end{equation}

\noindent que se debe a la estructura del vértice fundamental de QED, el cual siempre involucra un fotón. Si el fotón es una linea interna de un diagrama éste debe estar asociado a dos vértices, mientras que si es una pata externa estará asociado a uno solo. También es posible expresar el número de vértices como
%
\begin{equation}
	V = P_e + \frac{N_e}{2}\,,
\end{equation}

\noindent que también responde a la estructura del vértice fundamental de QED.
...
\begin{IEEEeqnarray*}{rCl}
	...
	%%%%
	&=& 4 - 2N_e + \frac{N_e}{2} - N_\gamma \slj
	%%%%
	\IEEEeqnarraymulticol{3}{c}{\boxed{ D = 4 - \frac{3}{2}N_e - N_\gamma }\,. \IEEEyesnumber}
\end{IEEEeqnarray*}

Es decir, el grado superficial de divergencia depende únicamente del número de patas externas del diagrama.

\begin{cabox}
	El grado superficial de divergencia depende únicamente del número de patas externas del diagrama, no del número de propagadores.
\end{cabox}


\paragraph{Aplicación a una teoría escalar}
\vspace{2ex}

Consideremos una teoría escalar $\lambda \phi^n$, cuyo Lagrangiano tiene la forma
%
\begin{equation}
	\lagdn = \frac{1}{2} (\partial_\mu \phi)(\partial^\mu \phi) - \frac{1}{2} m\phi^2 - \frac{\lambda}{n!} \phi^n \,.
\end{equation}

En este caso sólo existe un tipo de campo $\phi$, por lo que no es necesario hacer la distinción entre los propagadores $P_\gamma$ y $P_e$, y tenemos que
%
\begin{equation}
	L = P - V + 1 \qc nV = N + 2P \,.
\end{equation}

Así, el grado superficial de divergencia es
%
\begin{equation}
	D = dL - 2P \,,
\end{equation}

\noindent donde $d$ es la dimensión de las integrales de momento -- que en el caso de QED es 4, pero en este caso es arbitraria. Es posible mostrar que para una teoría escalar con un término de interacción $\lambda \phi^n$ este grado superficial de divergencia puede expresarse como
%
\begin{equation}
	D = d + \qty[n \qty(\frac{d-2}{2}) - d]V - \frac{d-2}{2}N \,.
\end{equation} \\

\paragraph{Una nota sobre unidades naturales}
\vspace{2ex}

Recordemos la dimensión que tienen algunas cantidades en unidades naturales. Al definir unidades naturales, la constante de Planck $\hbar = 1$, es decir tiene dimensión cero en unidades naturales. Como $\hbar$ tiene unidades de acción, esto hace que la acción tenga dimensión cero en unidades naturales, $[S] = 0$, y como
%
\begin{equation*}
	S = \int \dd[4]{x} \lagdn \,,
\end{equation*}

\noindent la dimensión del Lagrangiano debe ser $-4$ en unidades de longitud,
%
\begin{equation*}
	[\lagdn] = -4 \,,
\end{equation*}

Por su parte, el momentum lineal,
%
\begin{equation*}
	\vb*{p} = m \vb*{v} 
\end{equation*}

\noindent tiene unidades de masa puesto que, en unidades naturales, la velocidad es adimensional en virtud de que $c = 1$. Adicionalmente, como $v = \dd x / \dd t$, esto implica que la distancia y el tiempo tienen las mismas unidades. 
...
\noindent es adimensional, por lo que la posición $x$ tiene unidades de $p^{-1}$.

Volviendo a la acción, vemos entonces que el diferencial $\dd[4]x$ tiene magnitud $-4$ en unidades de momento, por lo que la densidad lagrangiana, en unidades de momento tiene magnitud
%
\begin{equation}
	[\lagdn] = 4 \qc \text{ en unidades de momento.}
\end{equation}

Siguiendo la definición de mecánica clásica, consideremos la densidad lagrangiana como la diferencia entre una densidad de energía cinética y una densidad de potencial $\lagdn = \mathcal{T} - \mathcal{V}$. Consideremos un potencial de la forma \textcolor{red}{***** Unificar convenciones de potencial, con o sin mathcal********}
%
\begin{equation}
...
Ahora, el Lagrangiano contiene el término $-m^2 \phi^2$, cuya magnitud debe ser 4 -- puesto que $[\lagdn] = 4$ --. Considerando que $[m] = 1$, tenemos que $[\phi] = 1$. Así, para el término en el potencial tenemos
%
\begin{equation}
	\qty[\frac{G_4}{4!} \phi^4] = [G_4] + 4 = 4\,,
\end{equation}

\noindent de modo que el acoplamiento $G_4$ debe ser adimensional. Entonces, para las magnitud de los acoplamientos tenemos que
%
\begin{equation}
	[G_n] = 4 - n \,.
\end{equation}
...
La idea, en general, será que las teorías que incluyen términos con potencias menores o iguales que $4$ de los campos tendrán buenas propiedades bajo renormalización, mientras que aquellas que incluyan términos con potencias mayores que $4$ presentarán complicaciones para su renormalización. 

En el caso de los fermiones, en la ecuación de Dirac aparece el término $-m\overline{\psi} \psi$ , que debe tener dimensión 4, de modo que
%
\begin{IEEEeqnarray*}{rCl}
...
%
\begin{equation*}
	J_\mu = \overline{\psi} \gamma^\mu \psi, \qquad [J_\mu] = 3 \,,
\end{equation*}
...
\noindent entonces la teoría de Fermi es no-renormalizable.
...
%------------------------------------------------------
\section{Renormalización y teoría de perturbaciónes}

Para obtener expresiones que resulten en amplitudes finitas, aún cuando tratemos con diagramas divergentes, el Lagrangiano ha de contener una masa desnuda $m_0$ y constantes de acoplamiento desnudas $\lambda_0$ que sean consistentes con la teoría hasta un límite superior $\Lambda$ de los momentos. Este Lagrangiano que incorpora tales cantidades se denomina \emph{Lagrangiano desnudo}. Consideremos el Lagrangiano
%
\begin{equation}
	\lagdn = \frac{1}{2} \partial_\mu\phi \partial^\mu\phi - \frac{1}{2}m_0^2 \phi^2 - \frac{\lambda_0}{4!} \phi^4 \,.
\end{equation}

El objetivo de la renormalización es absorber todas las constantes infinitas que aparezcan en las amplitudes divergentes dentro de los tres parámetros que no son observables, a saber, la masa desnuda, la constante de acoplamiento desnuda y la el campo. Para conseguir esto, es conveniente reformular la teoría de perturbación de forma que estas cantidades no-observables no aparezcan explícitamente en las reglas de Feynman. Iniciamos redefiniendo el campo $\phi$ como
%
\begin{equation}
	\phi = \sqrt{Z} \phi_r \,,    
\end{equation}

\noindent donde $Z$ es un parámetro denominado renormalización de la función de onda, y $\phi_r$ será el campo renormalizado. Con esto, el Lagrangiano es dado por
%
\begin{equation}
...
La masa y la constante de acoplamiento desnudas aún aparecen en esta expresión para $\lagdn$, pero pueden ser eliminadas definiendo:
...
\noindent donde $m$ y $\lambda$ son la masa y constante de acoplamiento físicas medibles, respectivamente. Con esto el Lagrangiano toma la forma
%
\begin{equation}
...
Los primeros tres términos de este Lagrangiano tiene la forma de una teoría $\phi^4$, pero ahora dependen de la masa y el de la constante de acoplamiento físicas. Los siguientes tres términos, que contienen las $\delta$'s se denominan \emph{contratérminos} y son ellas las que absorberán las diferencias infinitas y no-observables entre las cantidades desnudas y las cantidades físicas. Es tentador pensar que hemos agregado los contratérminos al Lagrangiano, pero de \eqref{eq:redef_Lag_renorm} vemos que simplemente las cantidades desnudas han sido divididas en dos componentes.
...
\begin{cabox}
	...
	\begin{itemize}
		\item Los campos que aparezcan en el término dan el grafo del diagrama de Feynman. Así, un término que incorpore, por ejemplo, $A^\mu \phi \partial_mu \phi$ representa un vértice en el que convergen un fotón (campo $A^\mu$) y dos campos escalares $\phi$.
	...
	Ahora, el factor $1/4!$ desaparece del factor de vértice debido a que hay 4 campos idénticos. Cuando se integra la acción, el hecho de tener un número $n$ de campos idénticos implicará que aparece un factor $n!$, que se va a cancelar exactamente con el denominador.
	
	Por otra parte, para el propagador del campo escalar nos remitimos a la parte del Lagrangiano que contiene únicamente el campo libre, esto es
	
	\begin{equation*}
		\lagdn_{\text{libre}} = \frac{1}{2}\partial_\mu \phi \partial^\mu \phi - \frac{1}{2} m^2 \phi^2 = \frac{1}{2} \phi \qty(\partial_\mu \partial^\mu - m^2)\phi \,.
	\end{equation*}

	En el espacio de momentos, 
	
	\begin{equation*}
		\qty(\partial_\mu \partial^\mu - m^2) \phi \, \longrightarrow \, \qty(p^2 - m^2)
	\end{equation*}

	\noindent y el propagador es
	
	\begin{equation*}
		\frac{\ii}{p^2 - m^2 + \ii \varepsilon} \,.
	\end{equation*}
\end{cabox}

\textcolor{red}{¿Cómo es esta parte?**************************}

Con esta prescripción es posible determinar los otros dos diagramas, que provienen de los contratérminos, hecho que es indicado con el símbolo $\otimes$ en el centro del diagrama. Para el primer diagrama de contratérminos tomamos los dos términos que incorporan dos campos en sus expresiones, i.e. $\frac{1}{2} \delta_Z \qty(\partial_\mu\phi_r)\qty(\partial^\mu\phi_r) - \frac{1}{2} \delta_m \phi_r^2$. Estos términos indican un campo libre. Para determinar el factor de vértice, no obstante, debemos realizar una modificación al primer término integrando por partes:
%
\begin{IEEEeqnarray*}{rCl}
...
\noindent con lo cual los contratérminos en el Lagrangiano que son bilineales en los campos son
%
\begin{equation}
...
El primer diagrama indica la forma que imponemos a la función de Green de dos puntos. La forma de la expresión indica que la amplitud invariante tendrá un polo en la masa física. El segundo diagrama indica la forma de la amplitud invariante para una dispersión dos-a-dos, donde $s,\, t,\,u$ son variables de Mandelstam dadas por
%
\begin{equation}
	\begin{IEEEeqnarraybox}[][c]{rCl}
		s &=& \qty(p_1 + p_2)^2 \,, \slj
...
\begin{IEEEeqnarray*}{c}
	...
	\label{eq:mandelstam_renorm}
\end{IEEEeqnarray*}

Lo que hemos hecho aquí para renormalizar es elegir un momentum $\mu$ (escala de renormalización) en el cual se evalúa la amplitud del diagrama. En este valor específico de momentum, tenemos libertad para elegir cuánto vale la amplitud. 

Ahora podemos usar las nuevas reglas de Feynman para determinar cualquier amplitud en una teoría $\lambda \phi^4$. El procedimiento es el siguiente: 

\begin{enumerate}
	\item Determinar la amplitud deseada como la suma de todos los diagramas posibles con los diagramas del propagador y de los vértices en la Fig.~\ref{fig:rFeynman_contraterminos}.
	
	\item Las integrales asociadas a los loops, generalmente, serán divergentes, de modo que necesitamos introducir un regulador. El resultado de este cálculo será una función con tres parámetros desconocidos $\delta_Z,\, \delta_m$ y $\delta_\lambda$.
	
	\item Ajustar (o ``renormalizar'') estos tres parámetros según sea necesario para mantener las condiciones de renormalización \eqref{eq:condRenorm_lambdaphi4}.
\end{enumerate}

   Después de este ajuste, la expresión para la amplitud debe ser finita e independiente del regulador.

Este procedimiento que emplea diagramas de Feynman con contratérminos se denomina \emph{teoría de perturbación renormalizada}.
...
Es conveniente hacer una pausa aquí para hacer una retrospectiva y comprender adecuadamente el proceso que hemos llevado a cabo y sus implicaciones:

\begin{cabox}
	Las cantidades a un loop aportan infinitos, pues resultan en las integrales divergentes que hemos denotado por $V$. Estas divergencias serán contrarrestadas con el valor del contratérmino $\delta_\lambda$ para ciertos valores específicos de los momentos dados en \eqref{eq:mandelstam_renorm}. Para estos mismos valores, hemos fijado el valor de la amplitud como siendo $\ii \lambda$, resultando en que el segundo término en \eqref{eq:renorm_amplitud} sea igual a cero.
	
	Para otros valores de los momentos, la amplitud invariante del diagrama a un loop, evidentemente, no será igual a cero. No obstante, el término ya no presenta infinitos y dará la evolución y comportamiento correctos de la amplitud $\aminv$ para valores arbitrarios de $s,\, t,\, u$. Esto es, al exigir que la amplitud sea finita en un punto del espacio de momentos se garantiza la convergencia de esta cantidad para cualesquiera valores arbitrarios de los momentos.
\end{cabox}
...
\begin{equation}
	\boxed{
	\frac{1}{A_1 A_2 \cdots A_n} = \int \dd{x_1} \dd{x_2} \cdots \dd{x_n} \delta\qty(\sum_i x_i - 1) \frac{(n-1)!}{\qty[x_1A_1 + x_2A_2 + \cdots + x_nA_n]^n}
	} \,.
\end{equation}
...
\begin{IEEEeqnarray*}{rCl}
	...
	%%%%
	&=& - \frac{(-\ii \lambda)^2}{2} \int \frac{\dd[4]{k}}{(2 \cpi)^4} \int_0^1 \dd x \frac{1}{\qty[x (k^2 - m^2) + (1 - x) \big( (k + p)^2 - m^2 \big)]^2} \,. \IEEEyesnumber \label{eq:calculo_FeynmanPar_1}
\end{IEEEeqnarray*}
...
\begin{IEEEeqnarray*}{rCl}
	...
	%%%%
	&=& k^2\qty(x + 1 -x) + 2k\vdot p \qty(1-x) + p^2(1-x) + m^2 \qty[-x -(1-x)] \\
	%%%%
