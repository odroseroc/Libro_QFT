...
\noindent que también responde a la estructura del vértice fundamental de QED. \textcolor{red}{[Ver si se pone toda la discusión sobre la forma de los vértices fundamentales de QED, o si se omite o se relega a otra sección del libro]}
...
Es decir, el grado superficial de divergencia depende únicamente del número de patas externas de cada tipo.

\begin{cabox}
	El grado superficial de divergencia depende únicamente del número de patas externas de cada tipo, no del número de propagadores.
\end{cabox}
...
\paragraph{Una nota sobre unidades naturales de ciertas cantidades}
\vspace{2ex}

Recordemos las dimensiones que tienen algunas cantidades en unidades naturales. Al definir unidades naturales, la constante de Planck $h = 1$, es decir tiene dimensión cero en unidades naturales. Como $h$ tiene unidades de acción, esto hace que la acción tenga dimensión cero en unidades naturales, $[S] = 0$, y como
%
\begin{equation*}
	S = \int \dd[4]{x} \lagdn \,,
\end{equation*}

\noindent la dimensión del Lagrangiano debe ser $-4$,
%
\begin{equation*}
	[\lagdn] = -4 \,,
\end{equation*}
...
Siguiendo la definición de mecánica clásica, consideremos la densidad lagrangiana como la diferencia entre una densidad de energía cinética y una densidad de potencial, $\lagdn = \mathcal{T} - \mathcal{V}$ para un potencial de la forma
%
\begin{equation}
...
Ahora, el Lagrangiano contiene el término cinético $-m^2 \phi^2$, cuya magnitud debe ser 4 -- puesto que $[\lagdn] = 4$ --. Considerando que $[m] = 1$, tenemos que $[\phi] = 1$. Así, para el término en el potencial tenemos
%
\begin{equation}
...
El objetivo de la renormalización es absorber todas las constantes infinitas que aparezcan en las amplitudes divergentes dentro de los tres parámetros que no son observables, a saber, la masa desnuda, la constante de acoplamiento desnuda y la intensidad del campo. Para conseguir esto, es conveniente reformular la teoría de perturbación de forma que estas cantidades no-observables no aparezcan explícitamente en las reglas de Feynman. Iniciamos redefiniendo el campo $\phi$ como
%
\begin{equation}
	\phi = \sqrt{Z} \phi_r \,,    
\end{equation}
...
\noindent donde $m$ y $\lambda$ son la masa y constante de acoplamiento físicas medibles. Con esto el Lagrangiano toma la forma
%
\begin{equation}
...
Los primeros tres términos de este Lagrangiano tiene la forma de una teoría $\phi^4$, pero dependiente de la masa y el acoplamiento físicos. Los siguientes tres términos, que contienen las $\delta$'s se denominan \emph{contratérminos} y son ellos los que han absorbido las diferencias infinitas y no-observables entre las cantidades desnudas y las cantidades físicas. Es tentador pensar que hemos agregado los contratérminos al Lagrangiano, pero de \eqref{eq:redef_Lag_renorm} vemos que simplemente las cantidades desnudas han sido divididas en dos componentes.
...
\begin{cabox}
	...
	Por su parte, para el propagador del campo escalar nos remitimos a la parte del Lagrangiano que contiene únicamente el campo libre, esto es
