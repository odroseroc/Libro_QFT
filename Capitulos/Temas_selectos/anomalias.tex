...
En este punto, debemos recordar que los fermiones en el loop contienen, tanto índices fermiónicos, como índices internos de los grupos de simetría. Por ejemplo, suponiendo que el fermión es un quark (up) (los bosones de las patas externas serían gluones), tendría un índice de color asociado a la simetría $\SU{3}$ que, en la representación fundamental, puede tomar el valor 1, 2 ó 3 y representan la carga de color (rojo, azul y verde) de un quark.

En la representación fundamental del grupo de generadores $\lambda^a_{ij}$ son matrices y por lo tanto, tienen dos índices $ij$ de color que se contraen con aquellos del fermión. En el ejemplo que estamos considerando, si a un vértice entra un quark con índice 1 (rojo) y sale otro quark con índice 3 (verde), el generador que origina esta transición tendría la forma (salvo por un factor de normalización)
%
\begin{equation*}
	\lambda^a = \pmqty{0 & 0 & 1 \\ 0 & 0 & 0 \\ 1 & 0 & 0} \,.
\end{equation*}

Así, el término en el lagrangiano para el acoplamiento de los quarks tendría la forma (salvo por un factor de normalización)

\begin{IEEEeqnarray*}{rCl}
	g\overline{u} \lambda^a u \gamma^\mu G_\mu &=& g \gamma^\mu_{\alpha \beta} \pmqty{\overline{u}_1 & \overline{u}_2 & \overline{u}_3}_\alpha \pmqty{0 & 0 & 1 \\ 0 & 0 & 0 \\ 1 & 0 & 0} \pmqty{u_1 \\ u_2 \\ u_3}_\beta G_\mu \slj
	%%
	&=& g \gamma^\mu_{\alpha\beta} G_\mu \overline{u}_{1\alpha} u_{3\beta} + \text{h. c.} \slj
	%%
	&=& g G_\mu \overline{u}_1 \gamma^\mu u_3 + \text{h. c.} \,,
\end{IEEEeqnarray*}

\noindent que es el acoplamiento requerido con su gluón correspondiente.

Si este es el acoplamiento en, por ejemplo, el vértice izquierdo, en el vértice superior debe aparecer el generador de la transición entre $q_3$ y el siguiente quark. En cada caso, el quark que sale del vértice anterior es el quark que entra al siguiente. Así, el segundo índice del generador del vértice anterior, debe ser igual al primer índice del generador del vértice siguiente, eso es:
...
\noindent por lo que el producto de generadores, para el diagrama que estamos estudiando, se reduce a una traza para un loop.
