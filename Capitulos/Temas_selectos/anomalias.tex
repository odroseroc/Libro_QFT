...
Consideremos el siguiente proceso a un loop:
...
Si este proceso fuese posible a nivel árbol, esperaríamos tener un término en el Lagrangiano de la forma
%
\begin{equation*}
	g A^\mu \partial_\nu A_\mu Z^\nu \,,
\end{equation*}

\noindent y de ser un término divergente, el infinito resultante puede ser absorbido por la constante de acoplamiento mediante el proceso de renormalización~\footnote{La renormalización es tratada en el Capítulo~\ref{ch:renormalizacion}}\,. Este proceso redefine las constantes de acoplamiento de tal forma que la teoría se vuelva coherente, consistente y no contenga infinitos. 

Ahora, si la amplitud del diagrama es infinita, pero el término con las patas externas consideradas no existe en el Lagrangiano, no es posible redefinir la teoría para hacerla coherente, i.e., existe una anomalía. Para que la teoría sea consistente, y no tenga infinitos sin cancelar, la única forma es que las contribuciones de cada fermión en el diagrama pueda ser cancelada. \\

\begin{definition}
	
	Definiremos una teoría que admite los siguientes vértices:
	%
	\begin{center}
...
Iniciaremos considerando corrientes neutras, de tal forma que no hay cambio de sabor en los vértices y, por tanto, el fermión en el loop es el mismo. Arbitrariamente, hemos elegido que el vértice de la derecha sea axial y los vértices de la izquierda sean vectoriales.

Nótese que el índice $c$ de los generadores del grupo gauge permanece asociado al bosón con momento $k_2$, mientras que el índice $b$ permanece asociado al bosón con momento $k_1$.

Sean $\epsilon^\mu,\, \quad \epsilon^{*\nu}\,, \quad \epsilon^{*\xi}$ las funciones de onda de los bosones involucrados. La amplitud invariante del diagrama directo puede denotarse siguiendo las reglas de Feynman: \textcolor{red}{******Verificar si los generadores no van dentro de una traza también******}
%
...
En el último paso, debemos tener en cuenta que las matrices $\lambda^i$ y las matrices $\gamma$ actúan en espacios diferentes, y por ello la traza puede descomponerse en el producto de las trazas sobre los generadores y sobre los términos que involucran matries $\gamma$.

De igual forma procedemos para el término cruzado:
%
\begin{IEEEeqnarray*}{rCl}
	\epsilon^\mu \epsilon^{*\nu} \epsilon^{*\xi} T^{abc}_{\mu \nu \xi} &=& \int \frac{\dd[4]{p}}{(2\pi)^4}
	\Tr \bigg[
...
Notemos que los términos exteriores a las integrales en \eqref{eq:anomalias1} y \eqref{eq:anomalias1Cruzado} son casi iguales, excepto por el orden en la multiplicación de los generadores $\lambda^i$. También existe un intercambio en los signos que acompañan a $k_1$ y a $k_2$ en el término cruzado, respecto al directo.

No vamos a resolver completamente estas integrales, pero bástenos con saber que son divergentes.

Vamos a manipular la traza en la integral del término directo.

\begin{profnoside}
	 Tomemos el caso del segundo término en la traza y multipliquemos y dividamos por $(\slashed{p} +m)$:
	%
	\begin{IEEEeqnarray*}{rCl}
		\frac{1}{\slashed{p} - m} \frac{\slashed{p} + m}{\slashed{p} + m} &=& \frac{\slashed{p} + m}{\slashed{p}^2 - m^2} \,,
	\end{IEEEeqnarray*}
	
	\noindent pero adicionalmente
	%
	\begin{IEEEeqnarray*}{rCl}
